...
def main(year, model, cuda_devices):
    print(f"Predict {year} with {model} using CUDA devices: {cuda_devices}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Predict scores using specified model and year.')
    parser.add_argument('--year', type=str, required=True, help='Year of the data to process')
    parser.add_argument('--model', type=str, choices=['roberta', 'xlnet'], required=True, help='Model to use for prediction')
    parser.add_argument('--cuda', type=str, default='0', help='CUDA devices to use (e.g., "0" or "0,1")')
    ...
    main(args.year, args.model, args.cuda)
